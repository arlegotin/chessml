...
Expected: the script consumes `pip freeze --all` and prints a `constraint-dependencies` property whose retained versions include `torch==2.7.0`, `torchvision==0.22.0`, `lightning==2.5.1.post0`, `pillow==9.5.0`, and `pytest==8.3.5`, and which omits ChessML, packaging tools, `torch-exid`, and `torchdata`.

Append the command's complete stdout beneath the final `[tool.uv]` table in `/tmp/chessml-lock-project/pyproject.toml` with `apply_patch`. Do not append it to the repository's canonical `pyproject.toml`.
