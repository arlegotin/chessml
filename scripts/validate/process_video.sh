...
board_width=$(( board_size * 5 / 2 ))
x_offset_right=$(( (desired_width * 3 / 4) - (board_width / 2) ))
...
for img in "$output_dir"/*.png; do
    ...
    read -r fen < "$board_img.txt"
    convert -size ${board_width}x${board_height} xc:black \
        \( "$board_img" -resize ${board_size}x -gravity south -geometry +0+0 \) -composite \
        "$temp_image"
    convert "$temp_image" -gravity north -pointsize 18 -fill white -annotate +0+0 "$fen" "$temp_image"
...
ffmpeg -framerate 24 -f image2pipe -i <(find "$combined_dir" -name "*.png" -print0 | sort -zV | xargs -0 cat) -c:v libx264 -pix_fmt yuv420p "${output_dir}_processed.mp4"
